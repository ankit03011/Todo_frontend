...
function UpdateTask({ getdata, props }) {
    /////////////PUT//////////////
    ...
    async function editTask(taskid) {
        let editApi = `http://localhost:5000/task/${taskid}`;
        let editTask = {
            task: UpdateTask,
            duedate: newdue,
    ...
    const call =()=>{toast.success(
        `Changes Saved Succefully
              ${props.task}
         You have To Complete This on ${moment(props.compdate).format('dddd','DD-MM-YYYY')}`,
        {
          duration: 6000,
        })}
    ...
    return (
        <div>
            <Toaster /> 
            <Button onClick={() => { update(props.task, props.duedate, props.compdate, props.status) }} style={{ marginLeft: '10px' }} variant="outline-dark" >Edit</Button>
            <div>
                <Modal style={{ zIndex: '2000' }} show={show} onHide={handleClose}>
                    ...
                    <Modal.Footer>
                        ...
                        <Button variant="primary" onClick={() => { editTask(props.taskid) }}>
                            Save Task
                        </Button>
                    </Modal.Footer>
